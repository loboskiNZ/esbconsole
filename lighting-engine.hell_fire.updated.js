...
sceneHell() {
    ...
    // Movers: solid red, shutter open, dimmer full, tilt "up" (~80), pan centered
    this.fixtures.movers.forEach(addr => {
    ...
    let tick = 0;
    const centerPan = 127;
    const panRange = 60; // amplitude: 127±60 => ~67..187
    const tiltUp = 80;
    ...
    this.hellInterval = setInterval(() => {
        ...
        this.fixtures.bars.forEach(addr => {
            for (let seg = 0; seg < 8; seg++) {
                const base = addr + (seg * 6);
                const c = this.randomFireColor();
                this.setBarPixel(animUpdate, base, c.r, c.g, c.b);
            }
        });
